...
summary(reg_dfw)
```
Yield ranging from 7.8 to 15.6 Mg/ha. Values are plausible.
...
Boxplot requirex x to be categorical but in this case we have seeding rate so this does not look good.
...
The relationship looks like quadratic.

Our goal is to then use that model to estimate the optimum seeding rate for this study.  If our goal was to estimate the mean effect of seeding rate on yield then we would conduct the analysis of variance by taking seeding rate as the categorical variable.
...
lm() runs fixed effects model only.
lmer() runs mixed effects (random effects + fixed effects) model

Fixed effects: X's are fixed
Random effects: 
...
Residuals are increasing in values as the fitted value increases. We do not want any clear pattern. 
...
ggplot(mod1_int_randeff, 
  ...
  stat_qq_line()
```
Few observations, nothing alarming. A horizontal line going from 0 in y_axis would mean that there is not much variation in y based on the given x values.
...
re.form = NA will not include random effects parts of the model in the prediction.
...
ggplot(reg_dfw,
  ...
  geom_line(data = nd,aes(y = mod1_yield_mgha), color = "forestgreen"
                )+
              geom_ribbon(data = nd,
...
# Model fitting
mod2_lin <- lmer(yield_mgha ~ sr_ksha + (1|rep), data = reg_dfw)
...
# Model fitting: Mixed effect model has fixed effects and random effects components
...
g <- lm(yield_mgha ~ sr_ksha + I(sr_ksha^2) , data = reg_dfw)
...
Quadratic because it fits the data well and we can also extract the maximum point.
...
# Model fitting
mod4_linp <- nlme(yield_mgha ~ SSlinp(sr_ksha, a, b, xs), data = reg_dfw, 
...
```
b and xs random estimates are so small that seem to be all zero.  For b and xs, there is not enough variation for random effect to be estimated.
...
Now the breakpoint is around 73 as we found in the summary of the model.
...
```
## Table comparison : IC_tab() comes from nlraa package
...
```
Based on the above, model 4 (linear-plateau) had the lowest AIC and thus should be used to find the optimum level of seeding rate. AIC goes on decreasing if the addition of variables is explaining variation in the data but if not then it increases. AICc is corrected AIC. This table sorts model on increasing order of AIC, BIC,.. and so on. If adding more parameters doesnot result in the reduction of AIC by at least 2 points then it is not worth it. It is better to use simpler models than a complex model that does not lead to enough reduction in AIC. Model that has larger weights fit the data better. 
...
Vertical line from x_axis to the optimum point and then horizontal line from the optimum point to the y-axis.
...
-   We use regression when both y and x are **numerical**. For categorical x's, we use analysis of variance. 
